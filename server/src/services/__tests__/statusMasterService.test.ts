...
describe('StatusMasterService', () => {
  ...
  it('creates a StatusMaster with defaults', async () => {
    const s = await service.create(
      { name: 'Pending', displayOrder: 1, isDefault: true, isTerminal: false },
      actorId
    );
  ...
  it('clears prior default when a new default is created', async () => {
    const a = await service.create({ name: 'A', isDefault: true }, actorId);
    const b = await service.create({ name: 'B', isDefault: true }, actorId);
    const refreshedA = await prisma.statusMaster.findUnique({ where: { id: a.id } });
    expect(refreshedA?.isDefault).toBe(false);
    expect(b.isDefault).toBe(true);
  });

  it('rejects duplicate status name (global unique)', async () => {
    await service.create({ name: 'Approved' }, actorId);
    await expect(
      service.create({ name: 'Approved' }, actorId)
    ).rejects.toThrow();
  });
  ...
  it('refuses to delete a StatusMaster referenced by a Claim', async () => {
    ...
    const s = await service.create({ name: 'InUse', isDefault: false }, actorId);
  ...
  it('refuses to delete a StatusMaster referenced only by a ClaimRemark (audit history)', async () => {
    ...
    const approved = await service.create({ name: 'Approved' }, actorId);
  ...
  it('list orders by displayOrder then name', async () => {
    await service.create({ name: 'Zebra', displayOrder: 1 }, actorId);
    await service.create({ name: 'Alpha', displayOrder: 1 }, actorId);
    await service.create({ name: 'Middle', displayOrder: 0 }, actorId);
  ...
  describe('default-status protection', () => {
    it('refuses to deactivate the default status via setStatus', async () => {
      const def = await service.create({ name: 'Pending', isDefault: true }, actorId);
