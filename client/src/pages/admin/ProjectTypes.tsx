...
export default function ProjectTypes() {
  ...
  const fetchData = async (page = 1, limit = 10) => {
    ...
    try {
      const response = await api.get<PaginatedResponse<ProjectType>>(
        `/admin/project-types?page=${page}&limit=${limit}`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedItem) {
        await api.put(`/admin/project-types/${selectedItem.id}`, formData);
        toast({ title: 'Success', description: 'Project type updated successfully' });
      } else {
        await api.post('/admin/project-types', formData);
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      await api.delete(`/admin/project-types/${selectedItem.id}`);
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/project-types/${selectedItem.id}/status`, { status: newStatus });
      toast({ title: 'Success', description: `Project type ${newStatus === 'ACTIVE' ? 'activated' : 'deactivated'} successfully` });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={handleCreate}><Plus className="mr-2 h-4 w-4" />Add Project Type</Button>
      </div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{selectedItem ? 'Edit Project Type' : 'Create Project Type'}</DialogTitle></DialogHeader>
      ...
      <ConfirmDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen} title="Delete Project Type" description={`Are you sure you want to delete "${selectedItem?.name}"? This action cannot be undone.`} confirmText="Delete" onConfirm={handleDelete} variant="destructive" isLoading={isSubmitting} />
      <ConfirmDialog open={isStatusOpen} onOpenChange={setIsStatusOpen} title={selectedItem?.status === 'ACTIVE' ? 'Deactivate Project Type' : 'Activate Project Type'} description={selectedItem?.status === 'ACTIVE' ? `Are you sure you want to deactivate "${selectedItem?.name}"? Users assigned will lose access.` : `Are you sure you want to activate "${selectedItem?.name}"?`} confirmText={selectedItem?.status === 'ACTIVE' ? 'Deactivate' : 'Activate'} onConfirm={handleStatusChange} variant={selectedItem?.status === 'ACTIVE' ? 'destructive' : 'default'} isLoading={isSubmitting} />
    </div>
  );
}
