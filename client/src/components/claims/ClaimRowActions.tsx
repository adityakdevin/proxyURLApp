...
interface ClaimRowActionsProps {
  ...
  canAct: boolean;
  /**
   * THIS claim's outstanding validation work, or null when there is none.
   *
   ...
   * blinks once and goes idle while the work it started has not begun.
   *
   * QUEUED matters as much as RUNNING and is the harder half: a drainer running two at a
   * time leaves everything else waiting, and until a validator writes to a status column
   * a queued claim is indistinguishable from an untouched one. Spinning for both is the
...
export function ClaimRowActions({
  ...
  return (
    <div className="flex items-center gap-0.5">
        ...
        title={
          busy
            ...
            : validationState === 'QUEUED'
              ? 'Queued — waiting for the validator'
      ...
      >
        {/* Spins for the request AND for everything it leaves outstanding. Tying it to the
            request alone was the first mistake (200ms flash); tying it to the status columns
            was the second — those hold their old values until a validator starts writing, so
            a queued claim still looked idle. This reads the run itself.

            Deliberately NOT disabled while work is outstanding. Re-queueing mid-run is
            harmless — the drainer claims each run atomically — and a wedged or forgotten run
            is exactly when a reviewer needs the button most. Greying it out would take the
            retry away at the only moment it matters. */}
        {busy || validationState ? (
